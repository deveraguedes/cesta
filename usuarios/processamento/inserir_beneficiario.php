...
// Corrigido: pegar cod_usuario pela URL
...
<script type="text/javascript">
   ...
   // validações NIS, CPF, CPF responsável
   $("#nis").blur(function(){
      if ($('#nis').val() != ""){
         $.post("verifica_nis.php",
         { nis: $('#nis').val() },
         function(data){
            if (data == 1){
            ...
            if (data == 2){
               alert ("Esse NIS faz parte da composição familiar de um beneficiário da cesta de Pascoa.");
               $('#nis').val("").focus();                  
            }
         });
      }
   ...
   $("#cpf_responsavel").blur(function(){
      if ($('#cpf_responsavel').val() != ""){
         $.post("verifica_cpf_responsavel.php",
